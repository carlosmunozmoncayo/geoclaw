...
subroutine setaux(mbc,mx,my,xlow,ylow,dx,dy,maux,aux)
    ...
    !More locals for dummy aux array
    integer, dimension(8,2) :: neighborhood_idx
    integer :: kk,iidx,jidx
    ...
    !Fill dummy aux array to check ghost cell behavior
    !It just contains the minimum depth of the water at rest in the
    !neighborhood of each cell (including the cell itself).
    neighborhood_idx(:,1) = [-1,-1,-1,0 ,0, 1,1,1]
    neighborhood_idx(:,2) = [-1, 0, 1,-1,1,-1,0,1]
    do jj=1,my !Only iterate over interior cells
        do ii=1,mx
