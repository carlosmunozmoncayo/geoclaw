...
! Also calls movetopo if topography might be moving.
    ...
    use debug_module, only: debug_log
 ...
 603                    format('     reset s =',f9.2,'  h=',e11.3, ' B=',f8.2,&
    ...
    !Write out aux and q values at specific locations for debugging
    do i=1-mbc,mx+mbc ! Loop over all cells including ghost cells
        x = xlower + (i-0.5d0)*dx
        do j=1-mbc,my+mbc
            boundary_flag = 0
            y = ylower + (j-0.5d0)*dy
            ...
            if (i<=mbc .or. i>mx-mbc .or. j<=mbc .or. j>my-mbc) then
                    call debug_log(boundary_flag,t,x,y,q(:,i,j),aux(:,i,j))
            end if
        end do
    end do
