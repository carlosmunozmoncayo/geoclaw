module bouss_module
    ...
    integer :: boussEquations       ! Which equations? 0=SWE, 1=Madsen, 2=SGN, -1=EDC
    real(kind=8) :: boussMinDepth   ! depth below which to switch to SWE
    double precision :: boussEDCcsq        !Reference EDC Hyperbolic relaxation parameter
...
!======================================================================
 ...
 !  # comment lines starting with #:
    ...
    read(7,*) boussEDCcsq       ! Reference EDC Hyperbolic relaxation parameter
    read(7,*) boussEDCgamma     ! Use EDC to approximate SGN (3/2) or Sainte-Marie equations (2)
    
    if (boussEquations==1) B_param = 1.d0/15.d0  ! parameter in MS equations
    if (boussEquations==2) then
        alpha = 1.153  ! parameter in SGN equations
    end if
    ...
    enddo        
    !Initialize first and last ghost cells
    useBouss(-1) = useBouss(0)
    useBouss(mx+2) = useBouss(mx+1)
    !------------------------------------
...
!======================================================================
! Routines for Escalante-Dumbser-Castro equations
!======================================================================
...
subroutine riemann_normal_HypRel_EDC(mu, nv, grav, gamma, c_sq, &
   ...
   total_fluct(mu,1)=hr*ur**2+hr*pr+0.5d0*grav*hr**2-&
                  hl*ul**2-hl*pl-0.5d0*grav*hl**2+&
                  (grav*hbar+gamma*pbar)*(br-bl)
   total_fluct(nv,1)=ur*hr*vr-ul*hl*vl
   total_fluct(4,1)=ur*hr*wr-ul*hl*wl!-gamma*pbar !Handled with operator splitting
   total_fluct(5,1)=ur*hr*(pr+csqbar)-ul*hl*(pl+csqbar)-csqbar*ubar*(2*(br-bl)+(hr-hl))!+2*c_sq*wbar 
...
!    alpha = matmul(R_inv,total_fluct)
    R_inv(:,:) = 0.d0
   R_inv(:,1)=[ubar*CEbar+grav*hbar,2*(csqbar+pbar),&
      -2*grav*hbar*wbar, -2*grav*hbar*vbar, -ubar*CEbar+grav*hbar]*(0.5d0/CEbar**2)
   R_inv(1,mu) = -0.5d0/CEbar
   R_inv(5,mu) = 0.5d0/CEbar
   R_inv(4,nv) = 1.d0
   R_inv(3,4) = 1.d0
   R_inv(:,5)=[1.d0,-2.d0,-2*wbar,-2*vbar,1.d0]*(0.5d0/CEbar**2)


    alpha(1,1) = R_inv(1,1)*total_fluct(1,1) + R_inv(1,mu)*total_fluct(mu,1)+ &
                 R_inv(1,5)*total_fluct(5,1)
    alpha(2,1) = R_inv(2,1)*total_fluct(1,1)+R_inv(2,5)*total_fluct(5,1)
    alpha(3,1) =  R_inv(3,1)*total_fluct(1,1)+total_fluct(4,1)+&
                R_inv(3,5)*total_fluct(5,1)
    alpha(4,1) = R_inv(4,1)*total_fluct(1,1)+total_fluct(nv,1)+&
                R_inv(4,5)*total_fluct(5,1)
    alpha(5,1) = R_inv(5,1)*total_fluct(1,1)+R_inv(5,mu)*total_fluct(mu,1)+&
                R_inv(5,5)*total_fluct(5,1)
...
subroutine riemann_normal_HypRel_BBBD(mu, nv, grav, c_sq, &
    ...
    R_inv(:,:) = 0.d0
    R_inv(:,1) = [0.5d0*(grav*hbar+CEbar*ubar), csqbar+pbar, -grav*hbar*sgbar, &
        -grav*hbar*wbar, -grav*hbar*vbar, -grav*hbar*pbbar, 0.5d0*(grav*hbar-CEbar*ubar)]
    R_inv(:,1) = R_inv(:,1)/CEbar**2.d0
    R_inv(1,mu) = -0.5d0/CEbar
    R_inv(7,mu) = 0.5d0/CEbar
    R_inv(5,nv) = 1.d0
    R_inv(4,4) = 1.d0
    R_inv(3,5) = 1.d0
    R_inv(:,6)=[0.5d0,-1.d0,-sgbar,-wbar,-vbar, -pbbar, 0.5d0]/CEbar**2.d0
    R_inv(6,7) = 1.d0

    alpha(1,1) = R_inv(1,1)*total_fluct(1,1) + R_inv(1,mu)*total_fluct(mu,1)+ &
                 R_inv(1,6)*total_fluct(6,1)
    alpha(2,1) = R_inv(2,1)*total_fluct(1,1)+R_inv(2,6)*total_fluct(6,1)
    alpha(3,1) =  R_inv(3,1)*total_fluct(1,1)+total_fluct(5,1)+&
                R_inv(3,6)*total_fluct(6,1)
    alpha(4,1) = R_inv(4,1)*total_fluct(1,1)+total_fluct(4,1)+&
                R_inv(4,6)*total_fluct(6,1) 
    alpha(5,1) = R_inv(5,1)*total_fluct(1,1)+total_fluct(nv,1)+&
                R_inv(5,6)*total_fluct(6,1)
    alpha(6,1) = R_inv(6,1)*total_fluct(1,1)+R_inv(6,6)*total_fluct(6,1)+&            
                total_fluct(7,1)
    alpha(7,1) = R_inv(7,1)*total_fluct(1,1)+R_inv(7,mu)*total_fluct(mu,1)+&
                R_inv(7,6)*total_fluct(6,1)
    ...
    sw(1) = min(ul-sqrt(grav*hl+csq_l+pl),lamb1bar)
    sw(2:6) = ubar
    sw(7) = max(ur+sqrt(grav*hr+csq_r+pr),lamb7bar)

end subroutine riemann_normal_HypRel_BBBD

function Source_HypRel_EDC_1D(hw, hp, h, csq_space, gamma)
    implicit none
    double precision :: Source_HypRel_EDC_1D(2), hw, hp, h, csq_space, gamma, dry_tolerance
    
    if (abs(h) > dry_tolerance) then
        Source_HypRel_EDC_1D(1) = gamma*hp/h
        Source_HypRel_EDC_1D(2) = -2.d0*csq_space*hw/h
    end if
end function 
