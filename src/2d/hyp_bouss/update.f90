...
!     real(kind=8), intent(in) :: level
    ...
    real(kind=8) :: hubouss_sum(4), hubouss_f(4), hubouss_c(4) !For additional variables
...
!$OMP                    hbouss_sum,hubouss_f,hubouss_c, &
                    ...
                    do j = jplo-jlo+nghost+1, jphi-jlo+nghost+1
                        ...
                        do jco = 1, intraty(lget)
                            do ico = 1, intratx(lget)
                                ...
                                if (nvar .ge. 4) then
                                    hubouss_f(1) = alloc(iaddf(4,iff+ico-1,jff+jco-1,locf,mi))*capa
                                    hubouss_f(2) = alloc(iaddf(5,iff+ico-1,jff+jco-1,locf,mi))*capa
                                end if
                                if (nvar .ge. 6) then
                                    hubouss_f(3) = alloc(iaddf(6,iff+ico-1,jff+jco-1,locf,mi))*capa
                                    hubouss_f(4) = alloc(iaddf(7,iff+ico-1,jff+jco-1,locf,mi))*capa
                                end if
                                ...
                                etasum = etasum + etaf     
                                if (nvar .ge. 4) then
                                    hubouss_sum(1) = hubouss_sum(1) + hubouss_f(1)
                                    hubouss_sum(2) = hubouss_sum(2) + hubouss_f(2)
                                end if
                                if (nvar .ge. 6) then
                                    hubouss_sum(3) = hubouss_sum(3) + hubouss_f(3)
                                    hubouss_sum(4) = hubouss_sum(4) + hubouss_f(4)
                                end if
                            enddo
                        enddo
                        ...
                        if (nwet > 0) then
                            ...
                            hvc = (hc / hsum) * hvsum
                            if (nvar .ge. 4) then
                                hubouss_c(1) = (hc / hsum) * hubouss_sum(1)
                                hubouss_c(2) = (hc / hsum) * hubouss_sum(2)
                            end if
                            if (nvar .ge. 6) then
                                hubouss_c(3) = (hc / hsum) * hubouss_sum(3)
                                hubouss_c(4) = (hc / hsum) * hubouss_sum(4)
                            end if
                        else
                            hc = 0.0d0
                            huc = 0.0d0
                        ...
                        if (nvar .ge. 4) then
                            alloc(iadd(4,i,j,loc,mitot)) = hubouss_c(1) / capac 
                            alloc(iadd(5,i,j,loc,mitot)) = hubouss_c(2) / capac 
                        end if
                        if (nvar .ge. 6) then
                            alloc(iadd(6,i,j,loc,mitot)) = hubouss_c(3) / capac 
                            alloc(iadd(7,i,j,loc,mitot)) = hubouss_c(4) / capac 
                        end if
