...
c
     ...
     &                         fgout_write,fgout_grid, FGOUT_ttol
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      use bouss_module, only: boussTstart, boussTfinal, use_bouss
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
c
 20   if (ncycle .ge. nstop .or. time .ge. tfinal) goto 999
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !Turn on/off dispersive terms for BTEs
      if (time .ge. boussTfinal) then
         use_bouss = .false.
      else if (time .ge. boussTstart) then
         use_bouss = .true.
      endif

      if ((time .lt. boussTstart ).and.
     &   (time + possk(1) .ge. boussTstart)) then
         !Take small step to reach BTEs start time
         ! print *, '------------------------------------------------'
         ! print *, 'Forcing small time step start'
         ! print *, '------------------------------------------------'
         possk(1) = boussTstart - time
         do i = 2, mxnest
            possk(i) = possk(i-1) / kratio(i-1)
         enddo
      endif

      if ((time .lt. boussTfinal) .and. 
     &   (time + possk(1) .ge. boussTfinal)) then
         !Take small step to reach BTEs final time
         ! print *, '------------------------------------------------'
         ! print *, 'Forcing small time step end'
         ! print *, '------------------------------------------------'
         possk(1) = boussTfinal - time
         do i = 2, mxnest
            possk(i) = possk(i-1) / kratio(i-1)
         enddo
      endif
