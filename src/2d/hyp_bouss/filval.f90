...
subroutine filval(val, mitot, mjtot, dx, dy, level, time,  mic, &
    ...
    if (naux == 0) then
        ...
        if (aux_finalized .lt. 2) then ! coarse topo was at wrong time. redo
           ...
           !Depth-based transition to BTEs, reset bouss component of aux
           if (0==boussTransitionType) auxc(dr_index,:,:) = NEEDS_TO_BE_SET
...
!! also might need preicallCopy???
       ...
       if (naux .gt. 0) then 
             ...
             if (aux_finalized .lt. 2) then
                aux(1,:,:) = NEEDS_TO_BE_SET  ! reset entire aux array since topo moving
                !Depth-based transition to BTEs, reset bouss component of aux
                if (0==boussTransitionType) aux(dr_index,:,:) = NEEDS_TO_BE_SET
             end if
